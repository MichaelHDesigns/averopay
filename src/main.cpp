...
// Copyright (c) 2018 The AveroPay developers
...
const string strMessageMagic = "AveroPay Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    //PoS Fixed on Block 100 AveroPay
            ...
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                        ...
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            ...
                            BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                            {
                                ...
                                if (vtx[1].vout[i].scriptPubKey == pubScript)
                                {
                                    ...
                                    if (fDebug) printf("Masternode PoS payee found at block %d: %s who got paid %s AOP (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[1].vout[i].nValue / COIN).c_str(), paidAge, mn.nBlockLastPaid);
                    ...
                    if(!(foundPaymentAmount && foundPayee)) {
                        ...
                        CBitcoinAddress address2(address1);
                        if(fDebug) { printf("CheckBlock-POS() : Expected Masternode reward of: %ld\n", masternodePaymentAmount); }
                        if(fDebug) { printf("CheckBlock-POS() : Couldn't find masternode payment(%d|%ld) or payee(%d|%s) nHeight %ld. \n", foundPaymentAmount, masternodePaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight+1); }
                    ...
                    if(fDebug) { printf("CheckBlock-POS() : Is initial download, skipping masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
            ...
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                        {
                            ...
                            if (vtx[0].vout[i].scriptPubKey == pubScript)
                            {
                                int lastPaid = mn.nBlockLastPaid;
                                int paidAge = pindex->nHeight+1 - lastPaid;
                                if (fDebug) printf("Masternode PoW payee found at block %d: %s who got paid %s AOP (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[0].vout[i].nValue).c_str(), paidAge, mn.nBlockLastPaid);
                                if (paidAge < 150) // TODO: Probably make this check the MN is in the top 50?
                                {
                                    if (fDebug) printf("WARNING: This masternode payment is too aggressive and will not be accepted in v3+\n");
                                }
                    ...
                    if(!(foundPaymentAmount && foundPayee)) {
                        ...
                        if(fDebug) { printf("CheckBlock-POW() : Found masternode payment %ld\n", pindexBest->nHeight+1); }
                    }
                } else {
                    if(fDebug) { printf("CheckBlock-POW() : Is initial download, skipping masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // AveroPay: ask for pending sync-checkpoint if any
    ...
    {
         if (fDebug) printf("ProcessBlock: ORPHAN BLOCK, prev=%s\n", pblock->hashPrevBlock.ToString().substr(0,20).c_str());
        // ppcoin: check proof-of-stake
        if (pblock->IsProofOfStake())
        {
            // Limited duplicity on stake: prevents block flood attack
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AveroPay", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "AveroPay masternode platform 1";
        CTransaction txNew;
        txNew.nTime = 1533110271;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nTime    = 1533110271;
        block.nVersion = 1;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
		block.nNonce   = 5320801;
		if(fTestNet)
        {
            block.nNonce   = 5320801;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x389e04ce43b5d1fcb7d0e7f2660027ecc830470d7c807984f738995d93318e61"));
        block.print();
...
{
    //int64_t ret = blockValue * 5/10 ; // 50%
	int64_t ret = static_cast<int64_t>(blockValue * 5/10); //50%
